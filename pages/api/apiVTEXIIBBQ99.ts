...
async function mapVtexOrderTo99Minutos(vtexData: any, forcedDeliveryType?: string): Promise<any> {
  ...
  const orderDataWithShipment = {
    shipment: {
      ...
      deliveryType: deliveryType,
      sender: {
        firstName: process.env.SENDER_FIRST_NAME || "BBQGRILL",
...
export default async function handler(
  ...
  const VTEX_API_URL = `https://bbqgrill.myvtex.com/api/oms/pvt/orders/${orderId}`;
  const API_VTEX_TOKEN_BBQ = process.env.API_VTEX_TOKEN_BBQ;
  const API_VTEX_KEY_BBQ = process.env.API_VTEX_KEY_BBQ;

  if (!API_VTEX_TOKEN_BBQ || !API_VTEX_KEY_BBQ) {
    ...
    try {
      response = await fetch(VTEX_API_URL, {
        ...
        headers: {
          ...
          'X-VTEX-API-AppKey': API_VTEX_KEY_BBQ,
          'X-VTEX-API-AppToken': API_VTEX_TOKEN_BBQ,
        },
