...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  const VTEX_API_URL_BL = `https://blanik.myvtex.com/api/oms/pvt/orders/${orderId}`;
  ...
  try {
    const response = await fetch(VTEX_API_URL_BL, {
      method: 'GET',
      headers: {
        'Content-Type': 'application/json',
        'X-VTEX-API-AppKey': process.env.API_VTEX_KEY_BL || '', // Asegurate de que esta variable contenga la clave correcta
        'X-VTEX-API-AppToken': process.env.API_VTEX_TOKEN_BL || '', // Si necesitás un token adicional
      },
    });
