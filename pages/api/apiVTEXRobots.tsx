...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  if (!API_VTEX_TOKEN) {
    return res
      .status(500)
      .json({ message: "API_VTEX_TOKEN no está configurada en las variables de entorno" });
  }
  ...
  try {
    ...
    if (getAllPages === 'true') {
      ...
      for (let i = 0; i < numWindows; i++) {
        ...
        const url = `https://imegab2c.myvtex.com/api/oms/pvt/orders${queryString}`;
        ...
        if (data.paging && data.paging.pages > 1) {
          ...
          for (let pageNum = 2; pageNum <= totalPages; pageNum++) {
            ...
            const pageUrl = `https://imegab2c.myvtex.com/api/oms/pvt/orders${pageQueryString}`;
