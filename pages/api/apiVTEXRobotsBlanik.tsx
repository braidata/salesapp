...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  if (!API_VTEX_TOKEN_BL) {
    return res
      .status(500)
      .json({ message: "API_VTEX_TOKEN_BL no está configurada en las variables de entorno" });
  }
  ...
  try {
    ...
    if (getAllPages === 'true') {
      ...
      for (let i = 0; i < numWindows; i++) {
        ...
        const url = `https://blanik.myvtex.com/api/oms/pvt/orders${queryString}`;
        ...
        const response = await fetch(url, {
          ...
          headers: {
            ...
            "X-VTEX-API-AppToken": process.env.API_VTEX_TOKEN_BL || "",
          },
        });
        ...
        if (data.paging && data.paging.pages > 1) {
          ...
          for (let pageNum = 2; pageNum <= totalPages; pageNum++) {
            ...
            const pageUrl = `https://blanik.myvtex.com/api/oms/pvt/orders${pageQueryString}`;
      ...
      const VTEX_API_URL = `https://blanik.myvtex.com/api/oms/pvt/orders${queryString}`;
...
async function getDetailedOrders(ordersList: any[], shippingMethod?: string, paymentMethod?: string) {
  ...
  const detailedOrders = await Promise.all(
    ordersList.map(async (order: any) => {
      try {
        const detailUrl = `https://blanik.myvtex.com/api/oms/pvt/orders/${order.orderId}`;
