...
export default async function handler(req, res) {
  const headers = {
    ...
    'rut': process.env.RUT_EMPRESA_EMISORA_QA,
    'clave': process.env.CLAVE_EMPRESA_COTIZADOR_QA
  };
  ...
  try {
    ...
    if (req.method === 'GET' && req.query.tipo === 'unitario') {
      ...
      const response = await fetch(
        process.env.URL_SEGUIMIENTO_DETALLE_QA,
        {
          method: 'POST',
          headers: headers,
          body: JSON.stringify({
    ...
    if (req.method === 'POST' && req.query.tipo === 'masivo') {
      ...
      const response = await fetch(
        process.env.URL_SEGUIMIENTO_QA,
        {
          method: 'POST',
          headers: headers,
    ...
    if (req.method === 'POST' && req.query.tipo === 'dd') {
      ...
      const response = await fetch(
        process.env.URL_SEGUIMIENTO_DD_QA,
        {
          method: 'POST',
          headers: headers,
          body: JSON.stringify(body)
    ...
    if (req.method === 'POST' && req.query.tipo === 'redestinacion') {
      ...
      const response = await fetch(
        process.env.URL_SEGUIMIENTO_ET_QA,
        {
          method: 'POST',
          headers: {
