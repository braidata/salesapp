...
/* 
=== BATCH ORDER PROCESSOR API ===

PURPOSE:
Process specific VTEX order IDs through the same pipeline as apiAnalitica.
Perfect for targeted recovery of missing orders or updating specific orders.
...
{
  "orderIds": ["v12345678abc"]
}

// Process multiple orders (from dataComparator results)
POST /api/batchOrderProcessor
Content-Type: application/json
{
  "orderIds": [
...
// Process batch from missing orders (max 50)
POST /api/batchOrderProcessor
Content-Type: application/json
{
  "orderIds": ["order1", "order2", "...", "order50"]
}
...
=== RESPONSE EXAMPLE ===
{
  "success": true,
  "results": {
    "totalOrdersRequested": 4,
    "vtexOrdersProcessed": 3,
    "sapOrdersMatched": 2,
    ...
    "orderResults": [
      ...
      {
        "orderId": "v87654321def",
        "vtexStatus": "success",
        "sapStatus": "not_found",
        "processingTime": 3200
      },
      {
        "orderId": "v11223344ghi",
        "vtexStatus": "not_found",
        "sapStatus": "not_found",
        "errorMessage": "VTEX fetch failed: Order not found",
        "processingTime": 1800
      },
      {
        "orderId": "v55667788jkl",
        "vtexStatus": "success",
        "sapStatus": "success",
        "processingTime": 2850
      }
  ...
  "metadata": {
    ...
    "averageOrderProcessingTime": 2500,
    "efficiency": {
      "vtexSuccessRate": 75.0,
      "sapSuccessRate": 50.0,
...
1. **Individual Order Tracking**: Each order result tracked separately
2. **Same Logic as apiAnalitica**: Uses exact same processing functions
3. **Detailed Error Reporting**: Specific error messages per order
4. **Performance Metrics**: Processing time per order and overall
5. **Success Rate Analysis**: VTEX, SAP, and combined success rates

=== STATUS MEANINGS ===

VTEX/SAP Status:
- "success": Order found and processed successfully
- "not_found": Order not found in the system
- "failed": API call or processing failed (see errorMessage)

=== RATE LIMITING ===
- 15 requests per 15-minute window per IP
- Maximum 50 orders per batch
- Built-in delays between API calls (500ms VTEX, 2s SAP)
