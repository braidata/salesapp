...
async function fetchAuthCodeFromPay(orderId) {
  try {
    const response = await fetch(`/api/apiVTEXPaype?orderId=${orderId}`);
...
const VTEXStatusChangerForm = () => {
  ...
  const fetchOrder = async (e) => {
    ...
    try {
      ...
      const response = await fetch(`/api/apiVTEXpe?orderId=${orderId}`);
      const data = await response.json();
        ...
        const mappedData = {
          ...
          Nombre:
            data.clientProfileData?.firstName && data.clientProfileData?.lastName
              ? `${data.clientProfileData.firstName} ${data.clientProfileData.lastName}`
              : "N/A",
          DNI: data.clientProfileData?.document || "N/A",
          "RUC": data.clientProfileData?.corporateDocument || "N/A",
  ...
  const handleUpdateStatus = async (e) => {
    ...
    try {
      const response = await fetch("/api/apiVTEXStatusChangerpe", {
        method: "POST",
        headers: {
  ...
  return (
    ...
    >
      <h1 className="text-2xl font-bold mb-4 text-center text-gray-200">
        Consulta y Actualización de Pedido VTEX VENTUS Perú
      </h1>
